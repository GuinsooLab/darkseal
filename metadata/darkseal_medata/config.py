...
PROXY_CLIENTS = {
    'NEO4J': 'darkseal_medata.proxy.neo4j_proxy.Neo4jProxy',
    'ATLAS': 'darkseal_medata.proxy.atlas_proxy.AtlasProxy',
    'NEPTUNE': 'darkseal_medata.proxy.neptune_proxy.NeptuneGremlinProxy',
    'MYSQL': 'darkseal_medata.proxy.mysql_proxy.MySQLProxy'
}

PROXY_CLIS = {
    'MYSQL': 'darkseal_medata.cli.rds_command.rds_cli'
}
...
class Config:
    ...
    # LOG_CONFIG_FILE = 'darkseal_medata/logging.conf'
    ...
    # On User detail method, these keys will be added into darkseal_common.models.user.User.other_key_values
...
class MySQLConfig(LocalConfig):
    ...
    PROXY_HOST = '192.168.120.14'   # type: ignore
    PROXY_PORT = 3306   # type: ignore
    PROXY_USER = 'root'   # type: ignore
    PROXY_PASSWORD = 'Bi9shxo=='   # type: ignore
    PROXY_DB = 'sample'

    defalult_uri = 'mysql://{user}:{password}@{host}:{port}/{db}'.format(
        user=PROXY_USER,
    ...
    SQLALCHEMY_DATABASE_URI = os.environ.get('SQLALCHEMY_DATABASE_URI', defalult_uri)
