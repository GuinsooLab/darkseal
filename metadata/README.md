...
$ python3 darkseal_medata/metadata_wsgi.py
...
$ ## docker run -p 5002:5002 amundsendev/amundsen-metadata gunicorn --bind 0.0.0.0:5002 darkseal_medata.metadata_wsgi
...
$ gunicorn darkseal_medata.metadata_wsgi
```
...
By default, Metadata service uses [LocalConfig](darkseal_medata/config.py "LocalConfig") that looks for Neo4j running in localhost.
In order to use different end point, you need to create [Config](darkseal_medata/config.py "Config") suitable for your use case. Once config class has been created, it can be referenced by [environment variable](darkseal_medata/metadata_wsgi.py "environment variable"): `METADATA_SVC_CONFIG_MODULE_CLASS`

For example, in order to have different config for production, you can inherit Config class, create Production config and passing production config class into environment variable. Let's say class name is ProdConfig and it's in darkseal_service.config module. then you can set as below:

`METADATA_SVC_CONFIG_MODULE_CLASS=darkseal_service.config.ProdConfig`
...
If you would like to use Apache Atlas as a backend for Metadata service you will need to create a [Config](darkseal_medata/config.py "Config") as mentioned above. Make sure to include the following:
