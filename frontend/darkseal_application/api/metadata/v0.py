...
from darkseal_common.entity.resource_type import ResourceType, to_label

from darkseal_application.api.utils.search_utils import generate_query_json
from darkseal_application.log.action_log import action_logging

from darkseal_application.models.user import load_user, dump_user

from darkseal_application.api.utils.metadata_utils import is_table_editable, marshall_table_partial, \
...
from darkseal_application.api.utils.request_utils import get_query_param, request_metadata, request_search
...
def _get_table_endpoint() -> str:
    darkseal_service_base = app.config['METADATASERVICE_BASE']
    if darkseal_service_base is None:
        raise Exception('METADATASERVICE_BASE must be configured')
    return darkseal_service_base + TABLE_ENDPOINT
...
def _get_feature_endpoint() -> str:
    ...
    if darkseal_service_base is None:
        raise Exception('METADATASERVICE_BASE must be configured')
    return darkseal_service_base + FEATURE_ENDPOINT
...
def _get_dashboard_endpoint() -> str:
    ...
    if darkseal_service_base is None:
        raise Exception('METADATASERVICE_BASE must be configured')
    return darkseal_service_base + DASHBOARD_ENDPOINT
...
def popular_resources() -> Response:
    ...
    Schema Defined Here:
    https://github.com/lyft/amundsenmetadatalibrary/blob/master/darkseal_service/api/popular_tables.py
    """
    try:
...
def get_table_metadata() -> Response:
    ...
    Schema Defined Here: https://github.com/lyft/amundsenmetadatalibrary/blob/master/darkseal_service/api/table.py
...
def get_last_indexed() -> Response:
    ...
    Schema Defined Here: https://github.com/lyft/amundsenmetadatalibrary/blob/master/darkseal_service/api/system.py
    """
    try:
